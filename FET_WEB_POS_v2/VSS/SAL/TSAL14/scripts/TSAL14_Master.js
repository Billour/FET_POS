...
$(document).ready(function() {
	...
	$(':text[jqKey="txtMSISDN"]')
	    .blur(function() {
	        var IsNumber = Number($(this).val());
            if (isNaN(IsNumber) || Number($(this).val()) <= 0) {
                $(this).val('');
...
function getSIMCardNoData(o) {
	var tr = $(o).parents('tr').first();
	var rowJson = getRowData(tr);
	var SimCardNo = parseInt(tr.find(':text[jqKey="SIM_CARD_NO"]').val());
	rowJson.SIM_CARD_NO = SimCardNo.toString();
...
function confirmItems() {
	...
	$.each(objs, function() {
		...
		var txtSIMCardNo = tr.find(':text[jqKey="SIM_CARD_NO"]');
		if ($.trim(txtSIMCardNo.val()) == "" &&
		    (rowJson.PRODTYPENO == "20" || rowJson.PRODTYPENO == "21" || rowJson.PRODTYPENO == "25" || rowJson.PRODTYPENO == "26")) {
			alert(prod + '未輸入 SIM 卡卡號 !!');
			txtSIMCardNo.focus();
			checkOK = false;
			return;
		}
...
function settingItemRow(json, row) {
    ...
    tr.find('td[jqKey="SIM_CARD_NO"]').show();
...
function addProd() {
	...
	var r = window.showModalDialog('../TSAL01/TSAL01_AddProds.aspx', rowJson, 'dialogWidth:500px;dialogHeight:450px;status:no;resizable:yes;scroll:yes');
	if (r == null || typeof (r) == 'undefined' || r.length == 0)
		return;
