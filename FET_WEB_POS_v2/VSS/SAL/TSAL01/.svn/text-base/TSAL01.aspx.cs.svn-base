...
public partial class VSS_SAL_TSAL01_TSAL01 : BasePage
{
    /// <summary>
    /// 來源類型
    /// </summary>
	...
	private string CheckData()
	{
		// 檢查是否有快取資料
		string result = "";
		string sqlstr = string.Format("select POSUUID_MASTER from SALE_HEAD where MODI_USER={0} and STORE_NO = {1} and SALE_STATUS = '1'", OracleDBUtil.SqlStr(this.logMsg.MODI_USER), OracleDBUtil.SqlStr(this.logMsg.STORENO));
		OracleConnection conn = null;
		try
		{
			conn = OracleDBUtil.GetConnection();
	...
	{
		Response.Redirect("~/VSS/SAL/TSAL05/TSAL05.aspx");
	}
	#endregion
