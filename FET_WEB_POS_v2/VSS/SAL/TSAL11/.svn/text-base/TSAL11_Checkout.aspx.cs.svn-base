...
public partial class VSS_SAL_TSAL11_TSAL11_Checkout : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string INVOICE_NO = (String)o["INVOICE_NO"];
        string INVOICE_DATE = (String)o["INVOICE_DATE"];
        string INVOICE_TYPE = (String)o["INVOICE_TYPE"];
        ...
        {
            CheckOut(POSUUID_MASTER, items, UNI_NO, UNI_TITLE, REMARK, INVOICE_NO, INVOICE_DATE, INVOICE_TYPE);
        }
    }


    private void CheckOut(string POSUUID_MASTER, JArray items, string uni_no, string uni_title, string remark, string INVOICE_NO, string INVOICE_DATE, string INVOICE_TYPE)
    {
        ...
        try
        {
            ...
            //產生發票
            SetInvoiceOrReceipt(POSUUID_MASTER, INVOICE_NO, INVOICE_DATE, INVOICE_TYPE, trans);
        ...
        if (IsCheckOut)
        {
            try
            {
                ...
                //{
                //    url = PrintInventory(POSUUID_MASTER);
                //}
                collection(POSUUID_MASTER);
                //RECEIPT_URL = Collection_Receipt(POSUUID_MASTER);
    ...
    private void UpdateToCloseHead(string posuuid_master, OracleTransaction trans)
    {
        ...
        sqlstr = "update TO_CLOSE_HEAD set posuuid_master = :posuuid_master,status=3 where posuuid_detail = :posuuid_detail";
    ...
    /// <returns></returns>
    private void SetInvoiceOrReceipt(string POSUUID_MASTER, string INVOICE_NO, string INVOICE_DATE, string INVOICE_TYPE, OracleTransaction objTx)
    {
        ...
        sb.Append(" SALE_TYPE, ");
        sb.Append(" SALE_NO, ");
        sb.AppendFormat(" '{0}' as INVOICE_NO, '{1}' as INVOICE_DATE, '{2}' as INVOICE_TYPE ", INVOICE_NO, INVOICE_DATE, INVOICE_TYPE);
        sb.Append(" from  ");
        sb.Append(" sale_head ");
        ...
        {
            INV_TAX_DRA = dtINV.Select(" INV_TYPE = '1' and TAXABLE = 'Y'");
            string HOST_ID = StringUtil.CStr(head.Rows[0]["HOST_ID"]);
            RECDT = dtINV.Select(" INV_TYPE = '3' ");                                              //收據

            facade.InsertManualInvoice(head, INV_TAX_DRA, "1", HOST_ID, objTx);
    ...
    private string Collection_Receipt(string posuuid_master, string receiptname)
    {
        ...
        try
        {
            ...
            if (dir.Count > 0)
            {
                ...
                {
                    return_url = Request.ApplicationPath + filePath + "/" + fileName;
                }
            }
        }
        catch (Exception ex)
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                switch (fun_id)
                {
                    case "1":
                        ...
                        string spath = fet.DOMain(POSUUID_MASTER);
                        bill_dispatch_url = get_bill_dispatch_url("FET");
                        ...
                        break;
                    case "3":
                        SEEDNET_BILLING.Instant_SEEDNET_BILLING seednet = new SEEDNET_BILLING.Instant_SEEDNET_BILLING();
                        string spath2 = seednet.DOMain(POSUUID_MASTER);
                        bill_dispatch_url = get_bill_dispatch_url("SEEDNET");
                        using (System.IO.StreamReader sr = new System.IO.StreamReader(spath2))
                        {
                            postString = sr.ReadToEnd();
                        }
                        break;
                ...
                if (!string.IsNullOrEmpty(postString))
                {
                    using (System.Net.WebClient client = new System.Net.WebClient())
                    {
                        ...
                        byte[] responseData = client.UploadData("", "POST", postData);
    ...
    private void OtherService(string posuuid_master)
    {
        ...
        {
            conn = OracleDBUtil.GetConnection();
            OracleCommand cmd = new OracleCommand("PK_BILL.PayModeSplitOne", conn);
