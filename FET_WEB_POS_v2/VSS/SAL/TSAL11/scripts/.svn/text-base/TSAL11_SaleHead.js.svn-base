...
function getUNINo(invo) {
    var r = openDialogWindowByEncrypt("../TSAL01/TSAL01_checkIDNumber.aspx?date=" + Date() + "&INVOICE_NO=" + invo, 400, 200);
	if (r == null || typeof (r) == 'undefined' || r.length == 0) {
		$(':text[jqKey="txtUNI_NO"]').focus();
	} else {
		rt = r.split(";");
		$(':text[jqKey="txtUNI_NO"]').val(rt[0]);
		if (rt[1] != null)
			$(':text[jqKey="txtREMARK"]').val(rt[1]);
	}
}