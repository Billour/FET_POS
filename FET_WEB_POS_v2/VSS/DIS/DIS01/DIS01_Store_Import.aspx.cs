   ...
   private static string BATCH_NO;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       if (!Page.IsPostBack)
       {
          ...
          //取得空的資料表
          gvStore.DataSource = getGridViewDataSTORE();
          gvStore.DataBind();
    ...
    private DataTable getGridViewDataSTORE()
    {
       ...
       dtResult.PrimaryKey = new DataColumn[] { dtResult.Columns["STORENO"] };
       return dtResult;
    }
    protected void bindMasterDataProduct()
    {
       DataTable dtResult = new DataTable();
       if (ViewState["ImportTable_S"] == null)
       {
          dtResult = getGridViewDataSTORE();
       }
       else
       {
           dtResult = (DataTable)ViewState["ImportTable_S"];
       }
    ...
    protected void btnImport_Click(object sender, EventArgs e)
    {
        try
        {
           ...
           Session["BATCH_NO"] = "STORE;" + BATCH_NO;
           string FINC_ID = "DIS01_STORE_IMPORT";
           ...
           {
              string STORENO = StringUtil.CStr(TmpTable.Rows[i][0]);
              if (string.IsNullOrEmpty(STORENO)) { continue; }
           ...
           try
           {
               _DIS01_Facade.ImportHeadQuarter(_DIS01_DiscountMasterDataSet_DTO, "STORE", "");
           }
           catch (Exception ex)
           {
              throw ex;
           ...
           DataTable dtNew = _DIS01_Facade.GetImportTempData(BATCH_NO, FINC_ID, USER_ID, "STORE");
           ViewState["ImportTable_S"] = dtNew;
           this.gvStore.DataSource = dtNew;
    ...
    protected void btnCommit_Click(object sender, EventArgs e)
    {
       if (!string.IsNullOrEmpty(this.hdUploadBatchNo.Value))
       {
           try
           {
              //更新資料庫
               _DIS01_Facade = new DIS01_Facade();
              _DIS01_Facade.UpdateOne_UpLoadTempMethodSet(BATCH_NO, "DIS01_STORE_IMPORT", logMsg.MODI_USER);
    ...
    {
        if (ViewState["ImportTable_S"] != null)
            ViewState["ImportTable_S"] = null;
        this.gvStore.DataSource = null;
        this.gvStore.DataBind();
        btnCommit.Enabled = false;
    }
