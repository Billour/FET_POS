   ...
   private static string BATCH_NO;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       if (!Page.IsPostBack)
       {
          ...
          //取得空的資料表
          gvProduct.DataSource = getGridViewDataCUST();
          gvProduct.DataBind();

       }        
    }

    private DataTable getGridViewDataCUST()
    {
       DataTable dtResult = new DataTable();
       dtResult.Columns.Add("CUSTMO", typeof(string));
       dtResult.Columns.Add("RESULT", typeof(string));
       dtResult.PrimaryKey = new DataColumn[] { dtResult.Columns["CUSTMO"] };
       return dtResult;
    }
    ...
    {
       DataTable dtResult = new DataTable();
       if (ViewState["ImportTable_Cm"] == null)
       {
          dtResult = getGridViewDataCUST();
       }
       else
       {
           dtResult = (DataTable)ViewState["ImportTable_Cm"];
       }
    ...
    protected void btnImport_Click(object sender, EventArgs e)
    {
        try
        {
           ...
           Session["BATCH_NO"] = "CUSTMOBIL;" + BATCH_NO;
           string FINC_ID = "DIS01_CUST_IMPORT";
           ...
           {
              string CUSTMO = StringUtil.CStr(TmpTable.Rows[i][0]);
              if (string.IsNullOrEmpty(CUSTMO)) { continue; }

              //HSSFRow row = sheet.GetRow(i) as HSSFRow;
              //DataRow dataRow = Table.NewRow();
              DIS01_DiscountMasterDataSet_DTO.UPLOAD_TEMPRow dataRow = Table.NewUPLOAD_TEMPRow();

              dataRow.F1 = "CUSTMOBIL";
              dataRow.F2 = CUSTMO;
           ...
           try
           {
               _DIS01_Facade.ImportHeadQuarter(_DIS01_DiscountMasterDataSet_DTO, "CUSTMOBIL", "");
           }
           catch (Exception ex)
           {
              throw ex;
           ...
           DataTable dtNew = _DIS01_Facade.GetImportTempData(BATCH_NO, FINC_ID, USER_ID, "CUSTMOBIL");
           ViewState["ImportTable_Cm"] = dtNew;
    ...
    protected void btnCommit_Click(object sender, EventArgs e)
    {
       if (!string.IsNullOrEmpty(this.hdUploadBatchNo.Value))
       {
           try
           {
              //更新資料庫
               _DIS01_Facade = new DIS01_Facade();
              _DIS01_Facade.UpdateOne_UpLoadTempMethodSet(BATCH_NO, "DIS01_CUST_IMPORT", logMsg.MODI_USER);
