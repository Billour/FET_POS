...
public partial class VSS_CHK_CHK04 : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            //取得空的資料表
            grid.DataSource = new CHK04_SmallChange_DTO().SMALL_CHANGE;
            grid.DataBind();
    ...
    {
        string HOST_NO = CHK04_PageHelper.GetHostNo(logMsg.STORENO, logMsg.MACHINE_ID);
    ...
    private void BindData()
    {
        ...
        grid.DataSource = new CHK04_Facade().Query_SmallChange(this.txtTraneDateS.Text, this.txtTraneDateS.Text, strMachine, logMsg.STORENO);
        grid.DataBind();
    }
    ...
    {
        CHK04_SmallChange_DTO CHK04_DTO = new CHK04_SmallChange_DTO();
        CHK04_SmallChange_DTO.SMALL_CHANGEDataTable dt = CHK04_DTO.SMALL_CHANGE;
        CHK04_SmallChange_DTO.SMALL_CHANGERow dr = dt.NewSMALL_CHANGERow();

        CHK04_Facade facade = new CHK04_Facade();
        ...
        CHK04_DTO.AcceptChanges();

        //更新資料庫
        facade.AddNewOne_SmallChange(CHK04_DTO);
    ...
    protected void grid_InitNewRow(object sender, DevExpress.Web.Data.ASPxDataInitNewRowEventArgs e)
    {
        ...
        e.NewValues["HOST_NO"] = CHK04_PageHelper.GetHostNo(logMsg.STORENO, logMsg.MACHINE_ID);
        e.NewValues["BATCH_NO"] = CHK04_PageHelper.GetDataCount(logMsg.STORENO, logMsg.MACHINE_ID, StringUtil.CStr(e.NewValues["TRADE_DATE"])) + 1;
