...
namespace FET.POS.Model.Common
{
    public class CON16_PageHelper
    {
        ...
        public static string GetCutOffDate(string YYMM)
        {
            ...
            sb.Append("SELECT CUT_OFF_DATE FROM CUT_OFF_DATE ");
            sb.Append("WHERE 1 = 1 ");
            sb.Append(" AND CUT_YYMM = " + OracleDBUtil.SqlStr(YYMM));
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(@"SELECT DISTINCT M.CSM_STOCKCHKM_ID, M.STKCHKNO, M.STKCHK_TYPE, E.EMPNAME, E.EMPNO, 
                            M.STORE_NO, M.STKCHKDATE, S.STORENAME ,M.MODI_USER ,EE.EMPNAME  MODI_NAME
                       FROM CSM_STOCKCHK_M M , EMPLOYEE E , STORE S , EMPLOYEE EE 
                            WHERE 1 = 1 "
                    );
            sb.Append("  AND M.STKCHKDATE = " + OracleDBUtil.SqlStr(STKCHKDATE));
            sb.Append("  AND M.STORE_NO = " + OracleDBUtil.SqlStr(STORE_NO));
            sb.Append(@" 
        ...
        public static DataTable Query_StockChkD_ByKey(string STKCHK_D_ID)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT * ");
            sb.Append("FROM CSM_STOCKCHK_D ");
            sb.Append("WHERE CSM_STOCKCHKD_ID = " + OracleDBUtil.SqlStr(STKCHK_D_ID));
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT DISTINCT  MODI_DTM  FROM CSM_STOCKCHK_D WHERE CSM_STOCKCHKD_ID = " + OracleDBUtil.SqlStr(STKCHK_M_ID));
