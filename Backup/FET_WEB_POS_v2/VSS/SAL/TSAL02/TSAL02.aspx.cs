...
public partial class VSS_SAL_TSA02 : BasePage
{
	OrderedDictionary QryArgs = new OrderedDictionary();

    protected void Page_Load(object sender, EventArgs e)
    {
        if (logMsg.STORENO.ToUpper() != System.Web.Configuration.WebConfigurationManager.AppSettings["DefaultStore"].ToString())
	...
	{
		getSALE_PERSON();
		txtS_Date.Date = Convert.ToDateTime(DateTime.Now.Year.ToString() + "/" + DateTime.Now.Month.ToString() + "/01");
	...
	protected void btnChangeProd_Click(object sender, EventArgs e)
	{
		if (gvMaster.FocusedRowIndex > -1)
		{
			...
			//Response.Redirect("~/VSS/SAL/TSAL01/TSAL01.aspx?SRC_TYPE=TSAL021&PKEY=" + s);

            //Response.Redirect("~/VSS/SAL/SAL03/SAL03.aspx?SRC_TYPE=SAL02&PKEY=" + s);

            //**2011/04/21 Tina：傳遞參數時，要先以加密處理。
            string encryptUrl = Utils.Param_Encrypt("SRC_TYPE=SAL02&PKEY=" + s);
	...
	protected void btnCancelDetail_Click(object sender, EventArgs e)
	{
		...
		{
			string s = gvMaster.GetRowValues(gvMaster.FocusedRowIndex, "POSUUID_MASTER") as string;
			//Response.Redirect("~/VSS/SAL/SAL041/SAL04.aspx?SRC_TYPE=SAL02&PKEY=" + s);

            //**2011/04/21 Tina：傳遞參數時，要先以加密處理。
            string encryptUrl = Utils.Param_Encrypt("SRC_TYPE=SAL02&PKEY=" + s);
            Response.Redirect("~/VSS/SAL/SAL041/SAL04.aspx?Param=" + encryptUrl);
	...
	protected void Button1_Click(object sender, EventArgs e)
	{
		...
		{
			string s = gvMaster.GetRowValues(gvMaster.FocusedRowIndex, "POSUUID_MASTER") as string;
			//Response.Redirect("~/VSS/SAL/TSAL01/TSAL01.aspx?SRC_TYPE=TSAL02&PKEY=" + s);

            //**2011/04/21 Tina：傳遞參數時，要先以加密處理。
            string encryptUrl = Utils.Param_Encrypt("SRC_TYPE=TSAL02&PKEY=" + s);
    ...
    static public int getProdInfo(string PRODNO)
    {
        ...
        {
            //** 2011/03/02 Tina：註解，因為銷售交易查詢不需判斷選取的商品是否為折扣商品(IS_DISCOUNT = 'Y')
