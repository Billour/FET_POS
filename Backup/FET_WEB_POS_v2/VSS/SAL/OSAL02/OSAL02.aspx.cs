...
public partial class VSS_SAL_OSAL02 : BasePage
{
	...
	void getSALE_PERSON()
	{
        DataTable dtSalePerson = new OSAL02_Facade().getSalePersonInSaleHead(logMsg.STORENO);
	...
	{
		setQueryParams();
		DataTable dtResult = new OSAL02_Facade().getSALE_HEAD(QryArgs);
	...
	{
		setQueryParams();
		gvMaster.DataSource = new OSAL02_Facade().getSALE_HEAD(QryArgs);
		gvMaster.DataBind();
	}
    ...
    {
        setQueryParams();
        OSAL02_Facade facade = new OSAL02_Facade();
        DataTable dtExport =facade.getOLDPOS(QryArgs);
        ...
        //    );
        string filename = new Output().Print("XLS", "舊POS作廢<td></td><td>匯出時間：" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + "</td>", null, dtExport, null);
        Response.Redirect(Utils.CreateTamperProofDownloadXlsURL("OLDPOS.xls"));
    }
}
