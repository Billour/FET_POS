...
public partial class VSS_TSAL01_ProductsPopup : Page
{
	...
	private DataTable GetMasterData()
	{
		...
		return new Product_Facade().Query_Product(sProdNo, sProName, sProdType);
	}
	...
	protected void SetReturnValue(string value)
	{
		...
		//script.AppendFormat("returnValue = '{0}');{1}window.close();{1}", value, Environment.NewLine);
		script.AppendFormat("returnChoosed('{0}', '{1}');{2}", RetrunControlID, value, Environment.NewLine);
