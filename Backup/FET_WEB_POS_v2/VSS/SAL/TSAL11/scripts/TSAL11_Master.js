...
$(document).ready(function() {
	...
	$(':text[jqKey="txtInvoice_No"]')
	    .blur(function() {
	        if ($(this).val().length == 0)
	            return;
	        var INVOICE_TYPE = 3;
	        if ($(':radio[jqKey="rbINVOICE_TYPE3"]').attr('checked')==true)
	            INVOICE_TYPE = 4;
	        ajaxcheckManualInvoiceNo($(this).val(), INVOICE_TYPE);
	    });
...
function deleteItemRow() {
	...
	$.each(chks, function() {
		...
		if (rowJson != null && typeof (rowJson.HAS_ADDPROD) != 'undefined' && rowJson.HAS_ADDPROD == '1') {
...
function chooseProdNo(o) {
	// Popup 產品選擇視窗
    var r = openDialogWindowByEncrypt('../TSAL01/ProductsPopup.aspx?date=' + Date(), 550, 400);
...
function openInputIMEIDialog(o) {
	...
	var tr = $(o).parents('tr').first();
	var rowJson = getRowData(tr);
	var r = window.showModalDialog('../TSAL01/TSAL01_InputIMEI.aspx', rowJson, 'dialogWidth:450px;dialogHeight:300px;status:no;resizable:yes;scroll:yes');
...
function reassignElementID(gv) {
	...
	if (rows.length == 0) {
		...
		gv.parents('div').first().find(':button[jqKey="btnMixPromotion"]').removeAttr('disabled');
...
function mixPromotion() {
	...
	if (rowJson != null)
	{
	    if (typeof (rowJson.PROMOTION_CODE) == 'undefined' || rowJson.PROMOTION_CODE.length == 0) {
            ...
            promoCode = rowJson.PROMOTION_CODE;
            detailId = rowJson.POSUUID_DETAIL;        
        }
    }
    //else {
	//	alert('請輸入商品編號');
	...
	$.each($('#gvMaster tr[jqKey]'), function(i, r) {
		...
		if (json == null) return;
		if (json.PROMOTION_CODE == promoCode && json.POSUUID_DETAIL == detailId)
			prods += detailId + '|' + json.PRODNO + ';';
	});
	if (prods.length != 0)
		prods = prods.substr(0, prods.length - 1)    
	var strWin = '../SAL01/SAL01_choosePromotions.aspx?Promotion_Code=' + promoCode + '&Posuuid_Detail=' + detailId + '&PromotionProdList=' + prods;
	...
	for (var i = 1; i < arr.length; i++) {
	    if (i== 1) {
	        promoCode = arr[i].split('^')[0];
	    }
	    else {
		    prods[prods.length] = arr[i].split('^')[0];
		    price [price.length] = arr[i].split('^')[2];
		}
    }
    if ($.trim(promoCode) == '') {
        promoCode = prods[0];
	...
	ajaxRequestPromotionProducts(promoCode, detailId, prods.join(';'), price.join(';'));
}
...
function checkrbINVOICE_TYPE() {
    if ($(':radio[jqKey="rbINVOICE_TYPE2"]').attr('checked')==true) {
        $('span[jqKey="labVOUCHER_TYPE"]').text('二聯式發票');
        $(':text[jqKey="txtInvoice_No"]').removeAttr('disabled');
        $(':text[jqKey="txtUNI_TITLE"]').removeAttr('disabled');
        $(':text[jqKey="txtUNI_NO"]').removeAttr('disabled');
        $('#ctl00_MainContentPlaceHolder_txtInv_Date_I').removeAttr('disabled');
        $('#ctl00_MainContentPlaceHolder_txtInv_Date_B-1').removeAttr('disabled');
        $(':text[jqKey="txtInvoice_No"]').css('background-color', '#FFF');
        $(':text[jqKey="txtUNI_TITLE"]').css('background-color', '#FFF');
        $(':text[jqKey="txtUNI_NO"]').css('background-color', '#FFF');      
    }
    else if ($(':radio[jqKey="rbINVOICE_TYPE3"]').attr('checked')==true) {
        $('span[jqKey="labVOUCHER_TYPE"]').text('三聯式發票');
        $(':text[jqKey="txtInvoice_No"]').removeAttr('disabled');
        $(':text[jqKey="txtUNI_TITLE"]').removeAttr('disabled');
        $(':text[jqKey="txtUNI_NO"]').removeAttr('disabled');
        $('#ctl00_MainContentPlaceHolder_txtInv_Date_I').removeAttr('disabled');
    ...
    else if ($(':radio[jqKey="rbINVOICE_TYPEno"]').attr('checked')==true) {
        $('span[jqKey="labVOUCHER_TYPE"]').text('收據');
        $(':text[jqKey="txtInvoice_No"]').attr('disabled', 'disabled');
        $(':text[jqKey="txtUNI_TITLE"]').attr('disabled', 'disabled');
        $(':text[jqKey="txtUNI_NO"]').attr('disabled', 'disabled');
        $('#ctl00_MainContentPlaceHolder_txtInv_Date_I').attr('disabled', 'disabled');
        $('#ctl00_MainContentPlaceHolder_txtInv_Date_B-1').attr('disabled', 'disabled');
        $(':text[jqKey="txtInvoice_No"]').css('background-color', '#E7E7E7');
        $(':text[jqKey="txtUNI_TITLE"]').css('background-color', '#E7E7E7');
        $(':text[jqKey="txtUNI_NO"]').css('background-color', '#E7E7E7');
        $(':text[jqKey="txtInvoice_No"]').val('');
        $(':text[jqKey="txtUNI_TITLE"]').val('');
        $(':text[jqKey="txtUNI_NO"]').val('');
    } 
}
