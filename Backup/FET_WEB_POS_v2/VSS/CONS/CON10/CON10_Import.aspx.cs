...
public partial class VSS_CONS_CON10_Import : Popup
{
    private static string BATCH_NO;
    private static string FINC_ID = "CON10_IMPORT";
    ...
    {
        DataTable dtNew = new CON10_Facade().GetImportTempData_P(this.hdUploadBatchNo.Value, FINC_ID, logMsg.MODI_USER, "PRODUCT");
        this.gvProduct.DataSource = dtNew;
        this.gvProduct.DataBind();
    }
    protected void BindStoreData()
    {
        DataTable dtNewS = new CON10_Facade().GetImportTempData_S(this.hdUploadBatchNo.Value, FINC_ID, logMsg.MODI_USER, "STORE");
    ...
    protected void btnImport_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            CON10_CSM_RTNM_DTO.UPLOAD_TEMPDataTable Table = new CON10_CSM_RTNM_DTO.UPLOAD_TEMPDataTable();
            ...
            {
                CON10_CSM_RTNM_DTO.UPLOAD_TEMPRow dataRow = Table.NewUPLOAD_TEMPRow();

                    string PRODNO = TmpTable.Rows[i][0].ToString(); //.StringCellValue;
                    dataRow["F1"] = "PRODUCT";
                    dataRow["F2"] = PRODNO.ToString();               
                ...
                if (row1 != null)
                {
                    for (int j = row1.FirstCellNum; j < cellCount1; j++)
                    {
                        if (row1.GetCell(j) != null)
                            dataRow1[j] = row1.GetCell(j).ToString();
                    }
            ...
            CON10_CSM_RTNM_DTO.UPLOAD_TEMPDataTable Table1 = new CON10_CSM_RTNM_DTO.UPLOAD_TEMPDataTable();

            //將exl的每個值抓出來IVR_Code
            for (int i = 0; i <= TmpTable1.Rows.Count - 1; i++)
            {
                CON10_CSM_RTNM_DTO.UPLOAD_TEMPRow dataRow1 = Table1.NewUPLOAD_TEMPRow();

                string STORE = TmpTable1.Rows[i][0].ToString(); //.StringCellValue;
                dataRow1["F1"] = "STORE";
                dataRow1["F2"] = STORE.ToString();
            ...
            try
            {
                new CON10_Facade().ImportHeadQuarter(Table, Table1);
            }
            catch (Exception ex)
            {
                throw ex;
            ...
            DataTable dtNew = new CON10_Facade().GetImportTempData_P(BATCH_NO, FINC_ID, USER_ID, "PRODUCT");
            this.gvProduct.DataSource = dtNew;
            this.gvProduct.DataBind();

            DataTable dtNewS = new CON10_Facade().GetImportTempData_S(BATCH_NO, FINC_ID, USER_ID, "STORE");
            this.gvStore.DataSource = dtNewS;
            this.gvStore.DataBind();
    ...
    protected void btnCommit_Click(object sender, EventArgs e)
    {
        ...
        {
            //更新資料庫
            new CON10_Facade().UpdateOne_UpLoadTempMethodSet(BATCH_NO, "CON10_IMPORT", logMsg.MODI_USER);
