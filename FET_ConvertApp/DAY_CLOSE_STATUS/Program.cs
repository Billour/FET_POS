...
namespace DAY_CLOSE_STATUS
{
    class Program
    {
        ...
        {
            //初始化LOG
            OutputMsg("1.DAY_CLOSE_STATUS開始");
            OutputMsg("2.初始化LOG");
            ConvertLog con_log = new ConvertLog("DAY_CLOSE_STATUS");
            try
            {
                OutputMsg("3.執行PK_CONVERT.SP_DAY_CLOSE_STATUS");
                string sRet =PK_CONVERT_SP_DAY_CLOSE_STATUS();
                OutputMsg(sRet);
                OutputMsg("4.執行PK_CONVERT.SP_DAY_CLOSE");//*** 20110302 DAY_CLOSE 程式合併在此執行
                sRet = PK_CONVERT_SP_DAY_CLOSE();
                OutputMsg(sRet);
        ...
        static string PK_CONVERT_SP_DAY_CLOSE_STATUS()
        {
            ...
            try
            {
                ...
                OracleCommand oraCmd = new OracleCommand("PK_CONVERT.SP_DAY_CLOSE_STATUS");
            ...
            {
                objTX.Rollback();
                OutputMsg("3.執行PK_CONVERT.SP_DAY_CLOSE_STATUS，例外產生");
                throw ex;
            }
            finally
            {
        ...
        static string PK_CONVERT_SP_DAY_CLOSE()
        {
            ...
            {
                objTX.Rollback();
                OutputMsg("4.執行PK_CONVERT.SP_DAY_CLOSE，例外產生");
                throw ex;
            }
            finally
            {
