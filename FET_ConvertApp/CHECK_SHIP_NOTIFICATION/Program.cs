...
namespace CHECK_SHIP_NOTIFICATION
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            //初始化LOG
            OutputMsg("1.初始化LOG");
            ConvertLog cLog = new ConvertLog("CHECK_SHIP_NOTIFICATION");
            try
            {
                OutputMsg("2.執行PK_CONVERT.SP_CHECK_SHIP_NOTIFICATION()");
                //string sMsg = new RTN_NOTIFICATION_Facade().SP_RTN_NOTIFICATION();
                string sMsg = SP_CHECK_SHIP_NOTIFICATION();
                OutputMsg(sMsg);
        ...
        static string SP_CHECK_SHIP_NOTIFICATION()
        {
            ...
            try
            {
                ...
                OracleCommand oraCmd = new OracleCommand("PK_CONVERT.SP_CHECK_SHIP_NOTIFICATION");
