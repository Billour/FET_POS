...
namespace ETC_BILLING_PAPER
{
    class Program
    {
        ...
        {
            //2.初始化LOG
            ConvertLog log = new ConvertLog("ETC_BILLING_PAPER");
            ETC_BILLING_PAPER nb = new ETC_BILLING_PAPER(log);
            try
            {
                ...
                log.Success("ETC_BILLING_PAPER 處理完成");
            }
            catch (Exception ex)
            {
                nb.Rollback();
    ...
    public class ETC_BILLING_PAPER
    {
        #region Header Class
        public class Header
        {
            ...
            public Header()
            {
                ...
                //***20110323 因改抓昨日之前的資料，檔名改取昨日
                //this.FILE_NAME = "PAY_B01_" + DateTime.Now.ToString("yyyyMMdd") + "_01.TXT";
                this.FILE_NAME = "PAY_B01_" + DateTime.Now.AddDays(-1).ToString("yyyyMMdd") + "_01.txt";
                this.BATCH_NO = ETC_BILLING_PAPER.SEQNO();
                this.SEND_STATUS = "0";
        ...
        public class Data
        {
            ...
            public static List<Data> getDataList(OracleTransaction objTx)
            {
                ...
                //string sqlstr = "select SH.SALE_NO SALE_NO, SH.STORE_NO as store_no,SH.MACHINE_ID as Host_id,To_char(SH. TRADE_DATE,'YYYYMMDD')  as TXT_DATE,To_char(SH. TRADE_DATE,'HH24MISS')  as TXT_TIME,SD.ID as SALE_DETAIL_ID,SD.BARCODE1 As BARCODE1,SD.BARCODE2 As BARCODE2,SD.BARCODE3 As BARCODE3,Substr(SD.barcode1,1,6) As BILL_DATE, BD.AMOUNT as AMOUNT, ROWNUM as SEQ_NO,Bd.BILL_DISPATCH_ID , Bd.PAY_MODE_ID ,Bd.PAY_MODE_NAME from BILL_DISPATCH bd,  sale_head sh,sale_detail sd where bd.POSUUID_MASTER = sh.posuuid_master   and bd.SALE_DETAIL_ID = sd.id and bd.bill_type='4' and bd.status='T' ORDER BY SH.STORE_NO";
                ...
                try
                {
                    ...
                    {
                        Data data = new Data();
                        data.SALE_NO = dr["SALE_NO"].ToString();
                        data.STORE_NO = "R" + dr["STORE_NO"].ToString();
        ...
        ConvertLog log;
        public ETC_BILLING_PAPER(ConvertLog log)
        {
            this.log = log;
        }
        ...
        public void Load()
        {
            ...
            this.file_path = getFile_path();//***20110324
        ...
        public void Init()
        {
            ...
            //string sqlstr = "update BILL_DISPATCH set STATUS='T' where bill_type='4' and STATUS='0' and BILL_TX_TYPE='1'";
            string sqlstr = "update BILL_DISPATCH set STATUS='T' where bill_type='4' and STATUS='0' and BILL_TX_TYPE='1' and exists  (SELECT 'X' FROM SALE_HEAD WHERE posuuid_master=bill_dispatch.posuuid_master and   TRUNC(TRADE_DATE)<=TRUNC(SYSDATE-1))";
        ...
        public void Insert()
        {
            ...
            try
            {
                ...
                #region Insert Data
                sqlstr = "insert into ETC_BILLING_D (\"ETC_BFD_ID\",\"ETC_BFM_ID\",\"HEADER_FLG\",\"STORE_NO\",\"HOST_ID\",\"TXT_DATE\",\"TXT_TIME\",\"SALE_DETAIL_ID\",\"BILL_DATE\",\"AMOUNT\",\"BARCODE1\",\"BARCODE2\",\"BARCODE3\",\"DIFF_FLG\",\"SEQ_NO\",\"BILL_DISPATCH_ID\") ";
                sqlstr += "VALUES(:ETC_BFD_ID,:ETC_BFM_ID,:HEADER_FLG,:STORE_NO,:HOST_ID,:TXT_DATE,:TXT_TIME,:SALE_DETAIL_ID,:BILL_DATE,:AMOUNT,:BARCODE1,:BARCODE2,:BARCODE3,:DIFF_FLG,:SEQ_NO,:BILL_DISPATCH_ID)";
                ...
                #region 資料交接
                sqlstr = "update BILL_DISPATCH set STATUS='1' where bill_type='4' and STATUS='T'";
                cmd = new OracleCommand(sqlstr, conn, trans);
                cmd.ExecuteNonQuery();
        ...
        public void ExportFile()
        {
            ...
            //Header***20110324 GET YESTERDAY DATA
            //string headerString = "1B01" + "6F2".PadLeft(8, ' ') + "101".PadLeft(8, ' ') + DateTime.Now.ToString("yyyyMMdd") + "2" + "".PadLeft(71, ' ');
            string headerString = "1B01" + "6F2".PadLeft(8, ' ') + "101".PadLeft(8, ' ') + DateTime.Now.AddDays(-1).ToString("yyyyMMdd") + "2" + "".PadLeft(71, ' ');//***20110324

            writer.WriteLine(headerString);
            //Data
            foreach (Data data in dataList)
            {
                string dataString = "2" + data.TXT_DATE.PadLeft(8, '0') + data.BARCODE2.PadLeft(16, '0') + data.BARCODE3.PadLeft(15, '0') + data.SALE_NO.Substring(data.SALE_NO.Length - 5, 5).PadLeft(5, '0') + data.STORE_NO.PadLeft(6, ' ') + "".PadLeft(49, ' ');
                writer.WriteLine(dataString);
            }

            //Trailer

            string TrailerString = "3" + trailer.TOTAL_AMOUNT.ToString().PadLeft(14, '0') + (dataList.Count + 2).ToString().PadLeft(10, '0') + "".PadLeft(75, ' ');
